...
def ddim_steps(x, seq, model, b, x_cond, diffusion=None, **kwargs):
    ...
    with torch.no_grad():
        ...
        for i, j in tqdm.tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            # target_pose의 배치를 4로 조정
            if target_pose.size(0) != 4:
                current_batch_size = target_pose.size(0)
                if current_batch_size < 4:
                    target_pose = target_pose.repeat(4 // current_batch_size, 1, 1, 1)[:4]
                elif current_batch_size > 4:
                    target_pose = target_pose[:4]

            # xt와 target_pose의 채널 수 확인
            print("xt shape:", xt.shape)
            print("target_pose shape:", target_pose.shape)

            # xt와 target_pose의 채널 수를 맞추기 위해 결합
            if xt.shape[1] + target_pose.shape[1] != 23 and xt.shape[1] < 23:  # 23으로 조정
                xt = torch.cat([xt, torch.zeros(n, 23 - xt.shape[1], xt.shape[2], xt.shape[3]).to(xt.device)], dim=1)
            ...
            if len(x_cond) == 4:
                [_, _, ref, mask] = x_cond
                if xt.shape != mask.shape:
                    mask = mask.expand_as(xt)
                if xt.shape != ref.shape:
                    ref = ref.expand_as(xt)
